...
# Extract response and last user message from JSON
CLAUDE_RESPONSE=$(echo "$INPUT" | python3 -c "import sys, json; data=json.load(sys.stdin); print(data.get('response', ''))")
USER_MESSAGE=$(echo "$INPUT" | python3 -c "import sys, json; data=json.load(sys.stdin); print(data.get('userMessage', data.get('prompt', '')))")
